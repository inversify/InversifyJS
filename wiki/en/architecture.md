...
The Plan is generated by an instance of the Planner class. The Plan contains a reference to the context and a reference to the (root) Request. A request represents a dependency which will be injected into a Target. 
...
- The first child request represents a dependency on `FooInterface` and its target is the constructor argument named `foo`.
- The second child request represents a dependency on `BarInterface` and its target is the constructor argument named `bar`.
...
### Middleware Phase
If we have configured some Middlewares they will be executed just before the resolution Phase takes place. Middleware can be used to develop some browser extensions that will allow us to display the resolution plan using some data visualization tool like D3.js. This kind of tools will help developers to identify problems during the development process.
