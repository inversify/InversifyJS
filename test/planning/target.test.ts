...
describe('Target', () => {
  ...
  it('Should be able to identify named metadata', () => {
    const target1: Target = new Target(
      TargetTypeEnum.ConstructorArgument,
      'katana',
      'Katana',
      'primary',
  ...
  it('Should be able to identify multi-injections', () => {
    ...
    target1.metadata.push(
      new Metadata(METADATA_KEY.MULTI_INJECT_TAG, 'Katana'),
    );
  ...
  it('Should be able to identify tagged metadata', () => {
    ...
    const target3: Target = new Target(
      TargetTypeEnum.ConstructorArgument,
      'katana',
      'Katana',
    );
    ...
    const target4: Target = new Target(TargetTypeEnum.Variable, '', 'Katana');
    target4.metadata.push(new Metadata(METADATA_KEY.INJECT_TAG, 'Katana'));
    expect(target4.isTagged()).to.be.eql(false);

    const target5: Target = new Target(TargetTypeEnum.Variable, '', 'Katana');
    target5.metadata.push(
      new Metadata(METADATA_KEY.MULTI_INJECT_TAG, 'Katana'),
    );
    expect(target5.isTagged()).to.be.eql(false);

    const target6: Target = new Target(
    ...
    const target9: Target = new Target(TargetTypeEnum.Variable, '', 'Katana');
