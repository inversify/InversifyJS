...
describe("NotConfiguredScope", () => {
  it("Should throw error when access get", () => {
    ...
    expect(() => notConfiguredScope.get(null as any, null as any)).to.throw(`${ERROR_MSGS.INVALID_BINDING_TYPE} Sid`);
  });

  it("Should throw error when access set", () => {
    const notConfiguredScope = new NotConfiguredScope("Sid");
    expect(() => notConfiguredScope.set(null as any, null as any, null as any)).to.throw(`${ERROR_MSGS.INVALID_BINDING_TYPE} Sid`);
  });
