...
describe('Resolve', () => {
	...
	it('Should be able to resolve BindingType.Factory bindings', () => {
		...
		container
			...
			.toFactory<Katana>((theContext: interfaces.Context) => () => theContext.container.get<Katana>(katanaId));
	...
	it('Should be able to resolve plans with custom contextual constraints', () => {
		...
		container
			...
			.to(Katana)
			.when((request: interfaces.Request) => request.target.name.equals('katana'));

		container
			.bind<Weapon>(weaponId)
			.to(Shuriken)
			.when((request: interfaces.Request) => request.target.name.equals('shuriken'));
	...
	it('Should be able to resolve plans with multi-injections', () => {
		...
		class Ninja implements Warrior {
			public katana: Weapon;
			public shuriken: Weapon;
			public constructor(@multiInject('Weapon') @targetName('weapons') weapons: Weapon[]) {
	...
	it('Should throw an error if the @postConstruct method throws an error', () => {
		...
		expect(resolveInstance.bind(resolveInstance, Katana, [], () => null)).to.throw(
			'@postConstruct error in class Katana: Original Message'
		);
	});
