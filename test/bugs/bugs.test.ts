...
describe('Bugs', () => {
  ...
  it('Should not throw when args length of base and derived class match', () => {
    ...
    class SamuraiMaster extends Warrior {
      ...
      public constructor(@inject(TYPES.Rank) @named('master') rank: string, @inject(TYPES.Weapon) weapon: Weapon) {
        // length = 2
        super(rank);
        this.weapon = weapon;
      }
  ...
  it('Should be able to identify is a target is tagged', () => {
    ...
    class Base {
      ...
      public constructor(@unmanaged() baseProp: string) {
  ...
  it('Should detect missing annotations in base classes', () => {
    ...
    abstract class Warrior {
      ...
      public constructor(@unmanaged() katana: Katana) {
        this._katana = katana;
      }
